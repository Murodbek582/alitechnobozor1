...
function processAuth() {
    ...
    if (isLoginMode) {
        ...
        // === REAL-TIME ADMIN NOTIFICATION (cross-tab + same-tab) ===
        // 1. BroadcastChannel orqali (zamonaviy brauzerlar)
        broadcastUpdate('NEW_USER', { id: newUser.id, name, email, registeredAt: newUser.registeredAt });
        // 2. Storage event (eski usul, cross-tab fallback)
...
function renderAdminUsers() {
    ...
    if (uList) {
        if (!allUsers.length) {
            uList.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:2rem; color:var(--text2);">' + t('no_users') + '</td></tr>';
        } else {
            const now = Date.now();
            const ONE_DAY = 24 * 60 * 60 * 1000;
            const sortedUsers = [...allUsers].reverse(); // eng yangi birinchi
            let usersHtml = '';
            sortedUsers.forEach(function(u, idx) {
                ...
                if (u.registeredAt) {
                    try {
                        ...
                        isNew = (now - d.getTime()) < ONE_DAY;
                    } catch(ex) { regDate = u.registeredAt.slice(0, 16).replace('T', ' '); }
                }
                // Buyurtmalar soni
                var userOrderCount = orders.filter(function(o) { return o.userEmail === u.email; }).length;
                var newBadge = isNew
                    ? '<span style="background:linear-gradient(135deg,#2ecc71,#27ae60);color:#fff;font-size:0.65rem;font-weight:800;padding:2px 7px;border-radius:20px;margin-left:6px;vertical-align:middle;letter-spacing:0.5px;">YANGI</span>'
                    : '';
                var rowStyle = isNew ? 'background:rgba(46,204,113,0.06); border-left: 3px solid #2ecc71;' : '';
                ...
                usersHtml += '<tr style="' + rowStyle + '">' +
                    ...
                    '<td style="font-size:0.78rem; color:var(--text2);">' + regDate + '</td>' +
                    '<td>' +
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (phoneInput) {
        ...
        phoneInput.addEventListener('keydown', (e) => {
            // +998 qismini o'chirishni taqiqlaymiz
